...
public class B2bFacadeServiceImpl implements B2bFacadeService {
	...
	@Override
	public String deliverOrder(B2bMemberDto member, String orderNumber,
			String orders) {
		...
		// 已发货和已完成的订单不能发货
		if (flag
				&& (4 == orderEx.getOrderStatus() || 6 == orderEx
						.getOrderStatus())) {
		...
		if (flag) {
			...
			if (null == orders || "".equals(orders)) {
				for (B2bOrderGoodsDtoEx o : orderGoods) {
					...
					og.setBoxesShipped(o.getAfterBoxes() == 0 ? o.getBoxes()
							: o.getAfterBoxes());
					og.setWeightShipped(o.getAfterWeight() == 0 ? o.getWeight()
							: o.getAfterWeight());
	...
	@Override
	public String cancelOrder(B2bMemberDto member, String orderNumber,
			Boolean flage) {
		...
		if (orderLock instanceof Boolean) {
			...
			if (-1 != odex.getOrderStatus() && odex.getOrderStatus() < 3) {
				...
				for (int i = 0; i < orderGoods.size(); i++) {
					...
					if (i > 0
							&& goodsPk.equals(orderGoods.get(i - 1)
									.getGoodsPk())) {
						continue;
					}
				...
				if (null != odex.getPaymentType() && 3 == odex.getPaymentType()) {
					try {
						Map<String, Object> map = foreignBankService
								.delLoanOrder(orderNumber);
						if (null != map.get("code")
								&& !RestCode.CODE_0000.getCode().equals(
					...
					try {
						...
						try {
							Query querys = Query.query(Criteria.where(
									"orderNumber").is(odex.getOrderNumber()));
							mongoTemplate
