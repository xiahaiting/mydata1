...
//						weight);
	...
	public List<ExportOrderDto> exportOrderList(Map<String, Object> map) {
		...
		for(ExportOrderDto dto:list){
			...
			dto.setActualAmount(ArithUtil.add(ArithUtil.round(ArithUtil.mul((null==dto.getPresentPrice()?0d:dto.getPresentPrice())+otherPrice
					, 0d==dto.getAfterWeight()?dto.getWeight():dto.getAfterWeight()), 2), dto.getPresentTotalFreight()));
		}
		return list;
	}
		...
		if(null !=list && list.size()>0){
			for(B2bOrderGoodsDtoEx dto : list){
				...
				if(Block.CF.getValue().equals(dto.getBlock())){
					...
					dto.setPresentTotalPrice( ArithUtil.round(ArithUtil.mul(om.getPresentPrice()+adminFee+
							loadFee+packageFee, 
							(0d==om.getAfterWeight()?om.getWeight():om.getAfterWeight())), 2));
				}else{
					dto.setOriginalTotalPrice(ArithUtil.round(ArithUtil.mul(om.getOriginalPrice(), 
							om.getWeight()), 2));
					dto.setPresentTotalPrice( ArithUtil.round(ArithUtil.mul(om.getPresentPrice(), 
							(0d==om.getAfterWeight()?om.getWeight():om.getAfterWeight())), 2));
				}
