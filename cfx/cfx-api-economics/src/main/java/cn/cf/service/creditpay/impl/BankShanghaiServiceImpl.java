...
public class BankShanghaiServiceImpl implements BankShanghaiService {
	...
	public BankBaseResp searchBankCreditAmount(B2bCompanyDto company,
       ...
       //存前置通知
		try {
			...
			//存后置通知
			String rest = postUtils.post("MCAgrmtContrQuery1_1Op", data, dto.getBankAccountNumber());
			if (rest.equals("loginFail")){
				this.saveBankInfo("MCAgrmtContrQuery1_1Op",null,rest,null,"active","login");
			}else {
	...
	public BankBaseResp searchloan(B2bLoanNumberDto loanNumber) {
		...
		try {
			...
			//存后置通知
			String rest = postUtils.post("qryLoanApplyRes1_1Op", data, loanNumber.getCustomerNumber());
			if (rest.equals("loginFail")){
				this.saveBankInfo("qryLoanApplyRes1_1Op",null,rest,null,"active","login");
			}
				...
				if (null != doc) {
					...
					if ("0".equals(retCode)) {
						...
						if (null != loanNo && !"".equals(loanNo)) {
							...
							String id1 = this.saveBankInfo("loanApplyResSync1_2Op", null, data, null, "active", "loan");
							String rest2 = postUtils.post("loanApplyResSync1_2Op", data2, loanNumber.getCustomerNumber());
...
//									5 可疑-为系统之间通讯异常
	...
	public BankBaseResp searchRepayment(B2bLoanNumberDto loanNumber) {
		...
		try {
			...
			String rest = postUtils.post("repaymentResSync1_1Op", data, loanNumber.getCustomerNumber());
			if (rest.equals("loginFail")){
				this.saveBankInfo("qryLoanApplyRes1_1Op",null,rest,null,"active","login");
			}
			this.saveBankInfo("qryLoanApplyRes1_1Op", id, data, rest, "active", "repayment");
			if (null != rest) {
				Document doc = null;
				try {
					doc = DocumentHelper.parseText(rest);
	...
	public BankBaseResp payOrder(B2bOrderDtoMa order, B2bCompanyDto company,
		...
		try {
			...
			//存后置通知
			String rest = postUtils.post("PusReqExInfo1_1Op", data, cgDto.getBankAccountNumber());
			if (rest.equals("loginFail")){
				this.saveBankInfo("PusReqExInfo1_1Op",null,rest,null,"active","login");
			}
			...
			if (null != rest) {
				...
				if (null != doc) {
					...
					if (null != retCode && ("0".equals(retCode) || "TXBK100073".equals(retCode))) {
						...
						String rest2 = postUtils.post("applyFinancingLoan3_1Op", data2, cgDto.getBankAccountNumber());
						if (rest2.equals("loginFail")){
							this.saveBankInfo("applyFinancingLoan3_1Op",null,rest2,null,"active","login");
						}
						//存后置通知
	...
	public BankBaseResp cancelOrder(B2bLoanNumberDto loanNumber) {
		...
		try {
			...
			//存后置通知
			String rest = postUtils.post("PusReqExInfo1_1Op", data, loanNumber.getCustomerNumber());
			if (rest.equals("loginFail")){
				this.saveBankInfo("PusReqExInfo1_1Op",null,rest,null,"active","login");
			}
			...
			if (null != rest) {
				...
				if (null != doc) {
					...
					String errMsg = root.element("opRep").element("errMsg").getTextTrim();
					//TXBK300270已关闭
					if ("0".equals(retCode)||"TXBK300270".equals(retCode)) {
						resp.setCode(RestCode.CODE_0000.getCode());
					} else {
						resp.setCode(RestCode.CODE_Z000.getCode());
						resp.setMsg(errMsg);
	...
	public BankBaseResp agentPay(B2bLoanNumberDto loanNumber,
		...
		try {
			...
			String rest = postUtils.post("FeeCharge1_1Op", data, loanNumber.getCustomerNumber());
			//存后置通知
