...
//			gdto.setPledgeUsedAmount(ArithUtil.round(ArithUtil.add(null==gdto.getPledgeUsedAmount()?0d:gdto.getPledgeUsedAmount(), o.getLoanAmount()), 2));
	...
	private void successBill(B2bBillOrderDto order) {
		...
		if(null != inventoryList && inventoryList.size()>0){
			...
			if(null != om){
				...
				financeRecordsService.insertFinanceRecords(order.getOrderNumber(), order.getAmount(), order.getPurchaserPk(), order.getPurchaserName(), order.getSupplierPk(), order.getSupplierName(), 1, 1, purDes+details, order.getSerialNumber(), null, om.getEmployeePk(), om.getEmployeeName(), om.getEmployeeNumber());
				financeRecordsService.insertFinanceRecords(order.getOrderNumber(), order.getAmount(), order.getSupplierPk(), order.getSupplierName(), order.getPurchaserPk(), order.getPurchaserName(), 1, 5, supDes+details, order.getSerialNumber(), null, om.getEmployeePk(), om.getEmployeeName(), om.getEmployeeNumber());
			}
		}
	}
	@Override
